...
fun NavegacionControladaDeLaApp(){
    ...
    val tituloDePantalla = when (rutaActual) {
        "home" -> "Inicio"
        "rangos" -> "Rangos de Apagado"
        "historial" -> "Historial de Mediciones"
        "cortar" -> "Control de Energía"
        "horarios" -> "Horarios de Activacion/Suspension"
           else -> "Login"
    }
    ...
    Scaffold(
        ...
        bottomBar = {
            if (mostrarBarras) {
                BarraDeNavegacionDeBotones(navegacionControlada = navegacionControlada)
                }
            }
